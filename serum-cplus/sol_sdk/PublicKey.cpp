...
namespace solana
{
    ...
    {
        auto Y = FieldElement51::from_bytes(point);
        auto Z = FieldElement51::one();
        auto YY = Y.square();
        auto u = YY.sub(Z);
        auto v = YY.mul(EDWARDS_D).add(Z);

        auto v3 = v.square().mul(v);
        auto v7 = v3.square().mul(v);
        auto r = u.mul(v3).mul(u.mul(v7).pow_p58());
        auto check = v.mul(r.square());

        auto cmp_field_elements = [](const FieldElement51& a, const FieldElement51& b) {
            return a.to_bytes() == b.to_bytes();
        };
        
        bool correct_sign_sqrt = cmp_field_elements(check, u);
        bool flipped_sign_sqrt = cmp_field_elements(check, u.negate());
